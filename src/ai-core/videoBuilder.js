...
/* =========================
GET FFMPEG (With Render Crash Protection)
========================= */
function getFFmpeg() {
  if (ffmpegStatic && fs.existsSync(ffmpegStatic)) {
    console.log("Using ffmpeg-static:", ffmpegStatic);
    try {
      // CRITICAL FOR RENDER: Forces permission to execute the file
      fs.chmodSync(ffmpegStatic, 0o755); 
    } catch (e) {
      console.warn("Could not chmod ffmpeg, might already have permissions.");
    }
    return `"${ffmpegStatic}"`;
  }
...
export function buildVideo(images) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(audio)) return reject(new Error("voice.mp3 not found"));
      if (!images || images.length === 0) return reject(new Error("No images provided"));
      ...
      images.forEach((img, i) => {
        if (!fs.existsSync(img)) throw new Error(`Image missing: ${img}`);
      ...
      exec(commands, { maxBuffer: 1024 * 1024 * 200 }, (error, stdout, stderr) => {
        ...
        if (!fs.existsSync(output)) return reject(new Error("Video file not created"));
        ...
        CLEAN TEMP FILES
        ========================= */
        clipFiles.forEach(file => { if (fs.existsSync(file)) fs.unlinkSync(file); });
